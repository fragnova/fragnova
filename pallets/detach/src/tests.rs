...
mod process_detach_requests_tests {
	...
	fn eth_sign_detach_request(
		key_type: KeyTypeId,
		ecdsa_key: ecdsa::Public,
		detach_request: DetachRequest,
		nonce: u64,
	) -> Vec<u8> {
		let merkle_root: Hash256 =
			merkle_root::<Keccak256, _>(detach_request.collection.get_abi_encoded_hashes()).into();
		...
		let mut eth_signature = Crypto::ecdsa_sign_prehashed(
			...
			&keccak_256(
				&[
					...
					&keccak_256(
						&[
							&(detach_request.collection.get_type() as u8).to_be_bytes()[..],
							&merkle_root[..],
							&chain_id_be[..],
	...
	fn deterministically_compute_ecdsa_key(
		key_type: KeyTypeId,
		...
		let signature = Crypto::ed25519_sign(
			key_type,
			&ed25519_key,
			&[b"detach-ecdsa-keys", &ed25519_key.to_vec()[..]].concat(),
		)
		.unwrap();
	...
	fn add_detach_requests_to_local_storage(
		ext: &mut TestExternalities,
		process_detach_requests: &ProcessDetachRequests,
	) {
		ext.execute_with(|| {
			offchain_index::set(
				b"detach-requests",
				&process_detach_requests.detach_requests.encode(),
			);
		});
	...
	fn process_detach_requests_should_work() {
		...
		ext.execute_with(|| {
			let computed_ecdsa_key =
				deterministically_compute_ecdsa_key(KEY_TYPE, ed25519_public_key);
			...
			assert_eq!(
				...
				DetachInternalData {
					...
					merkle_root: merkle_root::<Keccak256, _>(
						process_detach_requests.detach_requests[0]
							.collection
							.get_abi_encoded_hashes()
					)
					.into(),
					target_chain: process_detach_requests.detach_requests[0].target_chain,
					target_account: process_detach_requests.detach_requests[0]
						.target_account
						.clone(),
			...
			assert!(<DetachInternalData::<<Test as SigningTypes>::Public> as SignedPayload::<
				Test,
			>>::verify::<crypto::DetachAuthId>(&data, signature));
		})
	}

	#[test]
	fn process_detach_requests_should_not_send_unsigned_transaction_if_no_ecdsa_key_in_the_keystore_under_key_type_is_an_ethereum_authority(
	) {
		// `key_type` mentioned in the test function name here is the constant `KEY_TYPE`
...
mod validate_unsigned_tests {
	...
	fn validate_unsigned_should_not_work_if_signer_is_not_a_detach_key() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(
				validate_unsigned_(&validate_unsigned),
