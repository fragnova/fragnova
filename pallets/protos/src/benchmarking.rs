...
benchmarks! {
	...
	detach {
		...
		Protos::<T>::add_eth_auth(RawOrigin::Root.into(), sp_core::ecdsa::Public::from_raw(public))?;

		let pre_len: usize = <DetachRequests<T>>::get().len();
	}: _(RawOrigin::Signed(caller), PROTO_HASH, SupportedChains::EthereumMainnet, target_account)
	verify {
	...
	// verify {
	// 	assert!(<DetachedProtos<T>>::contains_key(PROTO_HASH));
	// }
