...
benchmarks! {
	...
	}: upload(RawOrigin::Signed(caller), references, category, tags, linked_asset, license, data.clone(), None)
	verify {
	...
	detach {
		...
		Protos::<T>::upload(RawOrigin::Signed(caller.clone()).into(), references, Categories::Text(TextCategories::Plain), <Vec<Vec<u8>>>::new(), None, UsageLicense::Closed, immutable_data.clone(), None)?;
